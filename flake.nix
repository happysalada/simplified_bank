  ...
  outputs = { self, nixpkgs, rust-overlay, ... }:
    let
      package_overlay = final: prev: {
        simplied-bank = import ./package.nix { pkgs = final; };
      };
    ...
    in
    {
      ...
      packages = forAllSystems (system:
        {
          inherit (nixpkgsFor.${system}) simplified-bank;
        });
      defaultPackage = forAllSystems (system: self.packages.${system}.simplified-bank);
